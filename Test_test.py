...
class test_Main(unittest.TestCase):
    ...
    def test_1_win(self): # test case 1
        ...
        root = Tk() # generate new window
        app = Application(root)

        # assuming player clicks on skip button
        app.unload_title_screen()
        app.player_start_screen()
        
        app.check_input(1) # input one player
        self.assertTrue(app.in_game) # check if program is app.in_game

        # question 1
        app.check_input("b") # correct answer score+=10
        self.assertEqual(app.active_player.getScore(), 10)
        self.assertEqual(app.active_player.getTries(), 3)
        
        # question 2
        app.check_input("c") # correct answer score+=10
        self.assertEqual(app.active_player.getScore(), 20)
        self.assertEqual(app.active_player.getTries(), 3)

        # question 3-6
        app.check_input("a") # correct answer score+=5
        app.check_input("melanin") # correct answer score+=20
        app.check_input("a") # correct answer score+=10
        app.check_input("a") # correct answer score+=5

        # question 7
        app.check_input("kansas") # incorrect answer score-=5
        self.assertEqual(app.active_player.getScore(), 55)
        self.assertEqual(app.active_player.getTries(), 2)
        app.check_input("emerald city") # correct answer
        self.assertEqual(app.active_player.getScore(), 75)
        self.assertEqual(app.active_player.getTries(), 2)

        # question 8
        app.check_input("a") # correct answer score+=20
        app.check_input("yellow") # correct answer score+=20
        self.assertEqual(app.active_player.getTries(), 2)
        self.assertFalse(app.in_game) # program is no longer in game, player won
        #self.assert() # test that program displays win screen

        # return to main menu
        app.check_input("start")
        
        # check that the program is reset
        self.assertFalse(app.in_game)
        self.assertTrue(app.asking_num_players)

        # test completed, close window
        root.destroy()

    def test_2_lose(self): # start of test 2
        ...
        # question 1
        app.check_input("a") # incorrect answer x 3
        self.assertEqual(app.active_player.getScore(), -5)
        self.assertEqual(app.active_player.getTries(), 2)
        app.check_input("d")
        app.check_input("c")
        self.assertEqual(app.active_player.getScore(), -15)
        self.assertEqual(app.active_player.getTries(), 0)
        self.assertFalse(app.in_game) # program is no longer in game, player lost
